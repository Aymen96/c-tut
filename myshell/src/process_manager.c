...
int run_program(char *file_path, char *argv[])
{
    ...
    if((file_path == NULL) || (argv == NULL)) { // H: after passing pointer paramenters
        return ERROR_CODE;
    }
    child_pid = fork();
    if(child_pid == -1) { // H: after every fork
        return ERROR_CODE;
    } else if (child_pid == 0) {
        ...
        // allocate mem for the new argument vector
        // first deterrmine the length of argv
        while (argv[num++] != NULL);
        nargv = (char**)malloc(sizeof(char*) * (num + 1));
        if (nargv == NULL) { // H: after every malloc
            exit(ERROR_CODE);
        }
